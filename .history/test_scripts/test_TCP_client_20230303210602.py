...
data_type_convert = defaultdict(lambda:
                                ...
                                # to hex
                                # (original_fmt == 'str'     and target_fmt == 'hex'     ): 'a',
                                ('int'    , 'hex'    ): st.pack('>i', data).hex(),
                                ('uint16' , 'hex'    ): st.pack('>H', data).hex(),
                                ('uint32' , 'hex'    ): st.pack('>L', data).hex(),
                                ('float32', 'hex'    ): st.pack('>f', data).hex(),
                                ('float64', 'hex'    ): st.pack('>d', data).hex(),
                                # from hex
                                ('hex'    , 'str'    ): bytes.fromhex(data).decode('utf-8'),
                                ('hex'    , 'int'    ): st.unpack('>i', bytes.fromhex(data))[0],
                                ('hex'    , 'uint16' ): st.unpack('>H', bytes.fromhex(data))[0],
                                ('hex'    , 'uint32' ): st.unpack('>L', bytes.fromhex(data))[0],
                                ('hex'    , 'float32'): st.unpack('>f', bytes.fromhex(data))[0],
                                ('hex'    , 'float64'): st.unpack('>d', bytes.fromhex(data))[0]
                                })
