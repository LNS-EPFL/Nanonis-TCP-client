...
str_list = ['asd', 'dtgrw', 'eiuw']

import numpy as np

# create the bytes data
bytes_data = b'asd\x00\x00dtgrweiuw\x00'

# create a 1D array of fixed-length strings using numpy.frombuffer()
# convert the bytes back to a string array
str_array = np.frombuffer(bytes_data, '>3S')

print(np.frombuffer(b'\x00\x00\x80\x88', '>i')[0])
