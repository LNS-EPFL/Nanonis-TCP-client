...
def construct_header(command_name, body_size, res = True):
    ...
    bin_rep += dtype_converter('int', 'bin', body_size)         # boty size
    bin_rep += dtype_converter('uint16', 'bin', 1 if res else 0) # send response back (1) or not (0)
    bin_rep += b'\x00\x00'
    return bin_rep
...
print(len(construct_header('FolMe.XYPosGet', 15)))
# import struct
