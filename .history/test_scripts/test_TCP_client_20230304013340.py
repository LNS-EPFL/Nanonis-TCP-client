...
def dtype_cvt(original_fmt, target_fmt, data):
...
def header_construct(command_name, body_size, res = True):
    ...
    bin_rep += dtype_cvt('int', 'bin', body_size)         # boty size
    bin_rep += dtype_cvt('uint16', 'bin', 1 if res else 0) # send response back (1) or not (0)
    bin_rep += b'\x00\x00'
    return bin_rep
