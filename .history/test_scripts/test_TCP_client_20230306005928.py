...
def res_recv(sk, *varg_fmt, get_header = False, get_arg = True, get_err = False):
    ...
    if get_err:
        body_err_status = dtype_convert(res_bin_rep[0:4], 'bin', 'uint32')
...
def BiasSet(sk, bias):
    ...
    cmd = header+body
    cmd_send(sk, cmd)
    res_recv(sk, get_arg = False, get_err = True)
