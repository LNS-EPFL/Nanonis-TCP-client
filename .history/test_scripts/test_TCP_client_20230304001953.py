    ...
    if original_fmt in supported_formats and target_fmt in supported_formats:
        try:
            dtype_conv = defaultdict(lambda: None, 
                                            {
                                            # to hex
                                            ('str'    , 'hex'    ): lambda: st.pack('>'),
                                            ('int'    , 'hex'    ): lambda: st.pack('>i', data),
                                            ('uint16' , 'hex'    ): lambda: st.pack('>H', data),
                                            ('uint32' , 'hex'    ): lambda: st.pack('>L', data),
                                            ('float32', 'hex'    ): lambda: st.pack('>f', data),
                                            ('float64', 'hex'    ): lambda: st.pack('>d', data),
                                            # from hex
