...
def data_type_converter(original_fmt, target_fmt, data):
    data_type_convert = defaultdict(lambda:
                                    {
                                    (original_fmt == 'hex'     and target_fmt == 'str'     ): st.unpack(''),
                                    (original_fmt == 'hex'     and target_fmt == 'int'     ): st.unpack(),
                                    (original_fmt == 'hex'     and target_fmt == 'uint16'  ): st.unpack(),
                                    (original_fmt == 'hex'     and target_fmt == 'uint32'  ): st.unpack(),
                                    (original_fmt == 'hex'     and target_fmt == 'float32' ): st.unpack(),
                                    (original_fmt == 'hex'     and target_fmt == 'float64' ): st.unpack(),
                                    # (original_fmt == 'str'     and target_fmt == 'hex'     ): 'a',
                                    (original_fmt == 'int'     and target_fmt == 'hex'     ): st.pack(),
                                    (original_fmt == 'uint16'  and target_fmt == 'hex'     ): st.pack(),
                                    (original_fmt == 'uint32'  and target_fmt == 'hex'     ): st.pack(),
                                    (original_fmt == 'float32' and target_fmt == 'hex'     ): st.pack(),
                                    (original_fmt == 'float64' and target_fmt == 'hex'     ): st.pack(),
