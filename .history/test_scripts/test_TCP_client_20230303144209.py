...
def data_type_converter(original_fmt, target_fmt, data):
    ...
    if original_fmt and target_fmt in supported_formats:
        data_type_convert = defaultdict(lambda:
                                        ...
                                        (original_fmt == 'int'     and target_fmt == 'hex'     ): st.pack('>i', data)[0],
                                        (original_fmt == 'uint16'  and target_fmt == 'hex'     ): st.pack('>H', data)[0],
                                        (original_fmt == 'uint32'  and target_fmt == 'hex'     ): st.pack('>L', data)[0],
                                        (original_fmt == 'float32' and target_fmt == 'hex'     ): st.pack('>f', data)[0],
                                        (original_fmt == 'float64' and target_fmt == 'hex'     ): st.pack('>d', data)[0],
                                        # from hex
...
supported_formats = ['int', 'uint16', 'uint32', 'float32', 'float64', 'hex', 'str']
if original_fmt and target_fmt in supported_formats:
    print(bytes.fromhex('466F 6C4D 652E 5859 506F 7353 6574 0000 0000 0000 0000 0000 0000 0000 0000 0000').decode('utf-8'))
