...
data_type_convert = defaultdict(lambda: None,
                                ...
                                # to hex
                                # (original_fmt == 'str'     and target_fmt == 'hex'     ): 'a',
                                ('int'    , 'hex'    ): (lambda: st.pack('>i', data).hex())(2),
                                ('uint16' , 'hex'    ): (lambda: st.pack('>H', data).hex())(2),
                                ('uint32' , 'hex'    ): (lambda: st.pack('>L', data).hex())(2),
                                ('float32', 'hex'    ): (lambda: st.pack('>f', data).hex())(2),
                                ('float64', 'hex'    ): (lambda: st.pack('>d', data).hex())(2),
                                # from hex
                                ('hex'    , 'str'    ): (lambda: bytes.fromhex(data).decode('utf-8'))(2),
                                ('hex'    , 'int'    ): (lambda: st.unpack('>i', bytes.fromhex(data))[0])(2),
                                ('hex'    , 'uint16' ): (lambda: st.unpack('>H', bytes.fromhex(data))[0])(2),
                                ('hex'    , 'uint32' ): (lambda: st.unpack('>L', bytes.fromhex(data))[0])(2),
                                ('hex'    , 'float32'): (lambda: st.unpack('>f', bytes.fromhex(data))[0])(2),
                                ('hex'    , 'float64'): (lambda: st.unpack('>d', bytes.fromhex(data))[0])(2)
                                })

aaa = data_type_convert[(original_fmt, target_fmt)]
print(aaa)