...
def res_recv(*varg, return_header = False):
    # res_bin_rep = socket.receive(512)
    res_bin_rep = b'Bias.Set\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x04\x00\x01\x00\x00@\xc0\x00\x00'

    res = []
    if return_header:
        header_cmd_name = dtype_convert(res_bin_rep[0:32], 'bin', 'str')
        header_body_size = dtype_convert(res_bin_rep[32:36], 'bin', 'int')
        res.append(header_cmd_name)
        res.append(header_body_size)
    for idx, arg in enumerate(varg):
        print(idx)
        res.append(dtype_convert(res_bin_rep[], ' bin', arg))
        
    # body_err_status = dtype_convert(res_bin_rep[], 'bin', 'uint32')
    # body_err_dsc_size = dtype_convert(res_bin_rep[], 'bin', 'int') # error description size
    # body_err_dsc = dtype_convert(res_bin_rep[], 'bin', 'str') # error description
