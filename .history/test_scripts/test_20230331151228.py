...
def bias_spectr_par_save(com, fdir, fname = ''):
    bias_par = {'BiasSpectrChs': com.BiasSpectrChsGet(),
                ...
                'UtilSessionPath': com.UtilSessionPathGet()
                }
...
def bias_spectr(com, par, data_folder, basename = '%Y%m%d_'):
    ...
    com.UtilSessionPathSet(check_dirs(par['UtilSessionPath'].loc['Session path', 0] + data_folder))
    data, parameters = com.BiasSpectrStart(1, basename)
    com.UtilSessionPathSet(par['UtilSessionPath'].values)
...
sess_path = connect.UtilSessionPathGet().loc['Session path', 0] + '\\STS'
# print(sess_path.loc['Session path', 0])
bias_spectr_path = check_dirs(sess_path)
print(bias_spectr_path)
connect.UtilSessionPathSet(bias_spectr_path, 0)
my_tcp.socket_close()