...
def unit_convert(data):
    ...
    for item in unit_list:
        if item in data:
            significand = float(''.join(char for char in data if char.isdigit()))
...
def cmd_construct(header_construct, body_construct):
    return

# todo: response message decode
def res_decode():
    return

# todo: decode response message
def res_recv():
    return
