...
dtype_cvt_dict = defaultdict(lambda: None, {
        (('1dstr', 'str'), 'bin'): lambda data: np.array(data, dtype = '>S').tobytes(),
        (('int', '1dint'), 'bin'): lambda data: np.array(data, dtype = '>i').tobytes(),
    })

key = ('1dint', 'bin')
value = dtype_cvt_dict[tuple([k for k in key])](data)
print(value)
