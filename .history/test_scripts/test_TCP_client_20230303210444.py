...
data_type_convert = defaultdict(lambda: None,
                                ...
                                # to hex
                                # (original_fmt == 'str'     and target_fmt == 'hex'     ): 'a',
                                (original_fmt == 'int'     and target_fmt == 'hex'     ): lambda: st.pack('>i', data).hex(),
                                (original_fmt == 'uint16'  and target_fmt == 'hex'     ): lambda: st.pack('>H', data).hex(),
                                (original_fmt == 'uint32'  and target_fmt == 'hex'     ): lambda: st.pack('>L', data).hex(),
                                (original_fmt == 'float32' and target_fmt == 'hex'     ): lambda: st.pack('>f', data).hex(),
                                (original_fmt == 'float64' and target_fmt == 'hex'     ): lambda: st.pack('>d', data).hex(),
                                # from hex
                                (original_fmt == 'hex'     and target_fmt == 'lambda: str'     ): bytes.fromhex(data).decode('utf-8'),
                                (original_fmt == 'hex'     and target_fmt == 'int'     ): lambda: st.unpack('>i', bytes.fromhex(data))[0],
                                (original_fmt == 'hex'     and target_fmt == 'uint16'  ): lambda: st.unpack('>H', bytes.fromhex(data))[0],
                                (original_fmt == 'hex'     and target_fmt == 'uint32'  ): lambda: st.unpack('>L', bytes.fromhex(data))[0],
                                (original_fmt == 'hex'     and target_fmt == 'float32' ): lambda: st.unpack('>f', bytes.fromhex(data))[0],
                                (original_fmt == 'hex'     and target_fmt == 'float64' ): lambda: st.unpack('>d', bytes.fromhex(data))[0]
                                })
print(data_type_convert[(original_fmt, target_fmt)])