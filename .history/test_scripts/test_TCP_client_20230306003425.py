...
# construct header
def header_construct(command_name, body_size, res = False):
...
def res_recv(*varg_fmt, return_header = False):
    ...
    for idx, arg_fmt in enumerate(varg_fmt):
        ...
        arg = dtype_convert(res_bin_rep[40 + arg_total_size: 40 + arg_total_size + arg_size], 'bin', arg_fmt)
        arg_total_size += arg_size                                           
        res.append(arg)
    print(res)
