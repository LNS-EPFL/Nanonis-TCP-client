    ...
    def dtype_cvt(self, data, original_fmt, target_fmt): # data_size is the length of an 1d array (8 for example)or the number of rows and columns of a 2d array ([2,2] for example) 
        ...
        dtype_cvt_dict = defaultdict(lambda: None, 
                                     ...
                                     # to binary
                                      ...
                                      # from binary
                                      ('bin', 'str'    ): lambda: np.frombuffer('>%ds' % len(data), data)[0].decode('utf-8'), # unpack a string with a certain length
                                      ('bin', 'int'    ): lambda: np.frombuffer('>i', data)[0], 
                                      ('bin', 'uint16' ): lambda: np.frombuffer('>H', data)[0],
                                      ('bin', 'uint32' ): lambda: np.frombuffer('>L', data)[0],
                                      ('bin', 'float32'): lambda: np.frombuffer('>f', data)[0],
                                      ('bin', 'float64'): lambda: np.frombuffer('>d', data)[0]
                                     })
