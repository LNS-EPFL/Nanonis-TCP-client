...
#################### basic functions for creating commands ####################
    ...
    def dtype_convert(self, data, original_fmt, target_fmt, data_size = None): # data_size is the length of an 1d array (8 for example)or the number of rows and columns of a 2d array ([2,2] for example) 
        supported_formats = ['bin', 'str', 'int', 'uint16', 'uint32', 'float32', 'float64', 
                             '1dstr', '1dint32', '1duint32', '1dfloat32', '1dfloat64', 
                             '2dfloat32', '2dstr']
        dtype_conv = defaultdict(lambda: None, 
                                {
                                # to binary
                                ('str'    , 'bin', None): lambda: bytes(data, 'utf-8'),
                                ('int'    , 'bin', None): lambda: st.pack('>i', data),
                                ('uint16' , 'bin', None): lambda: st.pack('>H', data),
                                ('uint32' , 'bin', None): lambda: st.pack('>L', data),
                                ('float32', 'bin', None): lambda: st.pack('>f', data),
                                ('float64', 'bin', None): lambda: st.pack('>d', data),
                                ('1dstr'  , 'bin', data_size): lambda: 
                                # from binary
                                ('bin', 'str'    , None): lambda: st.unpack('>%ds' % len(data), data)[0].decode('utf-8'), # unpack a string with a certain length
                                ('bin', 'int'    , None): lambda: st.unpack('>i', data)[0],
                                ('bin', 'uint16' , None): lambda: st.unpack('>H', data)[0],
                                ('bin', 'uint32' , None): lambda: st.unpack('>L', data)[0],
                                ('bin', 'float32', None): lambda: st.unpack('>f', data)[0],
                                ('bin', 'float64', None): lambda: st.unpack('>d', data)[0]
                                })
        if original_fmt in supported_formats and target_fmt in supported_formats:
            try:
                return dtype_conv[(original_fmt, target_fmt, data_size)]()
            except TypeError:
