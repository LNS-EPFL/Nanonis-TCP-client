...
######################################## Bias Spectroscopy Module #############################################
    ...
    def BiasSpectrTimingGet(self):
        ...
        self.tcp.print_err(res_err)
        timing_df = pd.DataFrame({'Z averaging time (s):': res_arg[],
                                  'Z offset (m):': res_arg[],
                                  'Initial settling time (s):': res_arg[],
                                  'Maximum slew rate (V/s):': res_arg[],
                                  'Settling time (s):': res_arg[],
                                  'Integration time (s):': res_arg[],
                                  'End settling time (s):': res_arg[],
                                  'Z control time (s):': res_arg[]}).T
        print('\n'+
              timing_df.to_string(header=False)+
              '\n\nBias spectroscopy timing set.')
        return timing_df
