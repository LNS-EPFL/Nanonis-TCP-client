            ...
            for idx, arg_fmt in enumerate(varg_fmt):
                if arg_fmt == 'str':
                    ...
                    for ele_idx in range(num_of_ele): 
                        arg_ele_size = arg_size_dict['int'] # the size of the integer that indicate the string size
                        str_ele_size = self.dtype_convert(res_bin_rep[arg_byte_idx: arg_byte_idx + arg_ele_size], 'bin', 'int') # the size of the string element
                        arg_byte_idx += arg_ele_size

                        ele = self.dtype_convert(res_bin_rep[arg_byte_idx: arg_byte_idx + str_ele_size], 'bin', arg_fmt)
                        arg_byte_idx += str_ele_size
                        str_1d.append(ele)
                    ...
                    num_1d = []
                    arg_size = res_arg[idx-1] # array size in bytes
                    num_of_ele = arg_size/arg_size_dict[arg_fmt] #
                    arg_size_dict[arg_fmt]

                    for ele_idx in range(arg_size):
