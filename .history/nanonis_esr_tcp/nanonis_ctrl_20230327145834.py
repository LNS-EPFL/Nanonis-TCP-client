...
######################################## Generic Sweeper Module #############################################
    ...
    def GenSwpStart(self, get_data, sweep_dir, save_base_name_str_size, save_base_name, reset_signal):
        body  = self.tcp.dtype_cvt(get_data, 'uint32', 'bin')
        ...
        self.tcp.print_err(res_err)
        gen_swp_data_df = pd.DataFrame(res_arg[5].T, columns = res_arg[2][0])
        print('\n'+
              gen_swp_data_df.to_string(header=False)+
              '\n\nThe limits of the Sweep signals returned.')
        return gen_swp_data_df
