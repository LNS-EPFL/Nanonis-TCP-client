...
######################################## Follow Me Module #############################################
######################################## Tip Shaper Module #############################################
    def TipShaperStart(self, wait_until_fin, timeout_in_ms):
        timeout_in_ms = int(self.tcp.unit_cvt(timeout_in_ms)*1000)

        body  = self.tcp.dtype_cvt(wait_until_fin, 'uint32', 'bin')
        body += self.tcp.dtype_cvt(timeout_in_ms, 'int', 'bin')
        header = self.tcp.header_construct('TipShaper.Start', body_size = len(body))
        cmd = header + body

        self.tcp.cmd_send(cmd)
        _, _, res_err = self.tcp.res_recv()
        tipshaper_start_df = pd.DataFrame({'Wait until finished:': wait_until_fin,
                                           ''})
...
# ccc.BiasSpectrMLSModeGet()
# bias_start = tcp.unit_cvt([1e-9,2,89.8e-9])
ccc.BiasSpectrMLSValsGet()