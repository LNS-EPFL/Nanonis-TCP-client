...
#################### basic functions for creating commands ####################
    ...
    def dtype_cvt(self, data, original_fmt, target_fmt, *arg):  
        ...
        #* str to binary
        if original_fmt == 'str' or '1dstr' and target_fmt == 'bin': 
        np.array(data, '>S').tobytes(),
        #* int32 to binary
        if original_fmt == 'int' or '1dint' and target_fmt == 'bin': 
        np.array(data, '>i').tobytes(),
        #* unsigned int16 to binary
        if original_fmt == 'uint16' and target_fmt == 'bin': 
        np.array(data, '>H').tobytes(),
        #* unsigned int32 to binary
        if original_fmt == 'uint32' or '1duint32' and target_fmt == 'bin': 
        np.array(data, '>L').tobytes(),
        #* float32 to binary
        if original_fmt == 'float32' or '1dfloat32' or '2dfloat32' and target_fmt == 'bin': 
        np.array(data, '>f').tobytes(),
        #* float64 to binary   
        if original_fmt == 'float64' and target_fmt == 'bin': 
        np.array(data, '>d').tobytes(),
        #* binary to string (expression after '%' gives the size of the string in bytes)
        if original_fmt == 'bin' and target_fmt == 'str': 
        np.frombuffer(data[4:], '>%dS' % np.frombuffer(data[0:4], '>i')[0]), #
        #! binary to 1d string
        if original_fmt == 'bin' and target_fmt == '1dstr': 
        [].append(np.frombuffer(data, '>%dS' % len(data))),
        #* binary to int & 1d int
        if original_fmt == 'bin' and target_fmt == 'int' or '1dint': 
        np.frombuffer(data, '>i'),
        #* binary to unsigned int16 
        if original_fmt == 'bin' and target_fmt == 'uint16': 
        np.frombuffer(data, '>H'),
        #* binary to unsigned int32 and 1d unsigned int32
        if original_fmt == 'bin' and target_fmt == 'uint32' or '1duint32': 
        np.frombuffer(data, '>L'),
        #* binary to float32 and 1d float32
        if original_fmt == 'bin' and target_fmt == 'float32' or '1dfloat32': 
        np.frombuffer(data, '>f'),
        #* binary to 2d float32
        if original_fmt == 'bin' and target_fmt == '2dfloat32': 
        np.frombuffer(data, '>f').reshape(arg),
        #* binary to float64
        if original_fmt == 'bin' and target_fmt == 'float64': 
