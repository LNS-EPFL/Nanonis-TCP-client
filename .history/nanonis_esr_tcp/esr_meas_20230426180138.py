...
class esr_meas:
    ...
    def atom_pickup(self):
        def meas_dz(radius = 1.5e-9):
            ...
            x = xy.loc['X (m)']
            y = xy.loc['Y (m)']
            surrounding_xy_list_ = [[x-radius, y], [x+radius,y], [x, y-radius], [x, y+radius]]
            surrounding_z_list = []

            self.connect.FolMeXYPosSet(*surrounding_xy_list_[0], 1)
            ...
            z_sur_1 = np.mean(surrounding_z_list)
            delta_z_1 = z_cen - z_sur_1
