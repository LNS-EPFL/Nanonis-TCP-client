...
#################### basic functions for creating commands ####################
    ...
    # ! only support formats in supported_dtypes list
    def dtype_cvt(self, data, original_fmt, target_fmt, *varg): # data_size is the length of an 1d array (8 for example)or the number of rows and columns of a 2d array ([2,2] for example) 
        supported_dtypes = ['bin', 'str', 'int', 'uint16', 'uint32', 'float32', 'float64']
        dtype_cvt_dict = defaultdict(lambda: 
                                     {
                                     # to binary
                                      (original_fmt == 'str' or '1dstr' and target_fmt == 'bin'): np.array(data, '>S').tobytes(),
                                      (original_fmt == 'int' or '1dint' and target_fmt == 'bin'): np.array(data, '>i').tobytes(),
                                      (original_fmt == 'uint16' and target_fmt == 'bin'): np.array(data, '>H').tobytes(),
                                      (original_fmt == 'uint32'  or '1duint32' and target_fmt == 'bin'): np.array(data, '>L').tobytes(),
                                      (original_fmt == 'float32' or '1dfloat32' or '2dfloat32' and target_fmt == 'bin'): np.array(data, '>f').tobytes(),
                                      (original_fmt == 'float64' and target_fmt == 'bin'): np.array(data, '>d').tobytes(),
                                      # from binary
                                      (original_fmt == 'bin' and target_fmt == 'str'): np.frombuffer(data[4:], '>%dS' % np.frombuffer(data[0:4], '>i')[0]), # expression after '%' gives the size of the string in bytes
                                      (original_fmt == 'bin' and target_fmt == '1dstr'): np.frombuffer(data, '>%dS' % len(data)),
                                      (original_fmt == 'bin' and target_fmt == 'int' or '1dint'): np.frombuffer(data, '>i'), 
                                      (original_fmt == 'bin' and target_fmt == 'uint16'): np.frombuffer(data, '>H'),
                                      (original_fmt == 'bin' and target_fmt == 'uint32'  or '1duint32'): np.frombuffer(data, '>L'),
                                      (original_fmt == 'bin' and target_fmt == 'float32' or '1dfloat32'): np.frombuffer(data, '>f'),
                                      (original_fmt == 'bin' and target_fmt == '2dfloat32'): np.frombuffer(data, '>f').reshape((num_rows, num_cols)),
                                      (original_fmt == 'bin' and target_fmt == 'float64'): np.frombuffer(data, '>d')
                                     })
        if original_fmt in supported_dtypes and target_fmt in supported_dtypes:
            try:
                return dtype_cvt_dict[original_fmt, target_fmt][True]
            except TypeError:
