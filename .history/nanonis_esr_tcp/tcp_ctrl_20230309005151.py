...
#################### basic functions for creating commands ####################
    ...
    def dtype_cvt(self, data, original_fmt, target_fmt, *varg): # data_size is the length of an 1d array (8 for example)or the number of rows and columns of a 2d array ([2,2] for example) 
        ...
        dtype_cvt_dict = defaultdict(lambda: 
                                     {
                                     # to binary
                                      (original_fmt == 'str'     or '1dstr'                    and target_fmt == 'bin'): np.array(data, dtype = '>S').tobytes(),
                                      (original_fmt == 'int'     or '1dint'                    and target_fmt == 'bin'): np.array(data, dtype = '>i').tobytes(),
                                      (original_fmt == 'uint16'                                and target_fmt == 'bin'): np.array(data, dtype = '>H').tobytes(),
                                      (original_fmt == 'uint32'  or '1duint32'                 and target_fmt == 'bin'): np.array(data, dtype = '>L').tobytes(),
                                      (original_fmt == 'float32' or '1dfloat32' or '2dfloat32' and target_fmt == 'bin'): np.array(data, dtype = '>f').tobytes(),
                                      (original_fmt == 'float64'                               and target_fmt == 'bin'): np.array(data, dtype = '>d').tobytes(),
                                      # from binary
                                      (original_fmt == 'bin' and target_fmt == 'str'                   ): np.frombuffer(data, '>%dS' % len(data)),
