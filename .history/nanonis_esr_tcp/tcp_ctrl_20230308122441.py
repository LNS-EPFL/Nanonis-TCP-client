...
#################### basic functions for creating commands ####################
    ...
    def res_recv(self, *varg_fmt, get_header = True, get_arg = True, get_err = True):
        ...
        if get_header:
            header['commmand name'] = self.dtype_convert(res_bin_rep[0:32], 'bin', 'str').replace('\x00', '') # drop all '\x00' in the string
            header['body size'] = self.dtype_convert(res_bin_rep[32:36], 'bin', 'int')            
            res_arg.append(header)
