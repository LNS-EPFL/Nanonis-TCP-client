        ...
        if get_arg:
            ...
            for idx, arg_fmt in enumerate(varg_fmt):
                if arg_fmt == '1dstr':
                    ...
                    num_1d = []
                    array_size = res_arg[idx-1] # array size in bytes
                    if array_size % arg_size_dict[arg_fmt] == 0:
                        num_of_ele = array_size//arg_size_dict[arg_fmt] # number of elements in the array
                    else:
                        print(f'array size ({array_size}) is not a multiple of element size ({arg_size_dict[arg_fmt]}). Please check if "arg_fmt" was correctly selected.')
