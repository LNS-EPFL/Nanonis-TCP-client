...
######################################## Bias Spectroscopy Module #############################################
    ...
    def BiasSpectrLimitsSet(self, start_val, end_val):
        ...
        self.tcp.print_err(res_err)
        limit_df = pd.DataFrame({'Start value (V):': start_val, 
                                 'Stop value (V):': end_val, 
                                 },
                                 index=[0]).T
        print('\n'+
              limit_df.to_string(header=False)+
              '\n\nBias limits set.')
        return limit_df
    
    def BiasSpectrLimitsGet(self):
        header = self.tcp.header_construct('BiasSpectr.LimitsGet', body_size = len(body))
