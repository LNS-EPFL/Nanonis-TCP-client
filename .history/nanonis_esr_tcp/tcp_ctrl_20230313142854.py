        ...
        if get_arg:
            ...
            for idx, arg_fmt in enumerate(varg_fmt):
                if arg_fmt in arg_size_dict.keys():
                    ...
                    num_cols = res_arg[idx-1] if not varg_fmt[idx-1].startwith('1d') and not varg_fmt[idx-1].startwith('2d') else res_arg[len(varg_fmt) -1- next(i for i, x in enumerate(reversed(varg_fmt)) if not x.startswith('1d') and not x.startswith('2d'))]
