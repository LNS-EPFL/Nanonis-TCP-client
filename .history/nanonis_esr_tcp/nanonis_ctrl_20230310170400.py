...
######################################## Bias Spectroscopy Module #############################################
    ...
    def BiasSpectrStart(self, save_base_name, get_data = 1):
        ...
        self.tcp.print_err(res_err)
        bias_spectr = pd.DataFrame(np.array(res_arg[5]).T, columns = res_arg[2][0])
        print('STS done!')
        return bias_spectr
    ...
    def BiasSpectrPropsSet(self, save_all, num_sweeps, bw_sweep, num_pts, z_offset, auto_save, show_save_dialog = 2):
        z_offset = self.tcp.dtype_cvt(z_offset)
        ...
        header = self.tcp.heafer_construct('BiasSpectr.PropsSet', body_size = len(body))
        cmd = header + body
    ...
    def BiasSpectrPropsGet(self):
        ...
        _, res_arg, res_err = self.tcp.res_recv('uint16', 'int', 'uint16', 'int', 'int', 'int', '1dstr', 'int', 'int', '1dstr', )
