...
#################### basic functions for creating commands ####################
    ...
    def res_recv(self, *varg_fmt, get_header = True, get_arg = True, get_err = True):
        ...
        if get_header:
            header_cmd_name = self.dtype_convert(res_bin_rep[0:32], 'bin', 'str').replace('\x00', '') # drop all '\x00' in the string
        ...
        if get_arg:
            ...
            for idx, arg_fmt in enumerate(varg_fmt):
                if arg_fmt == 'str':
                    arg_size = arg_size_dict['int'] # the size of an integer 32 that give the size of the following string
                    str_size = self.dtype_convert(res_bin_rep[arg_byte_idx: arg_byte_idx + arg_size], 'bin', 'int') # the size of the string 
                    ...
                    str_1d = []
                    array_size = res_arg[idx-2] # number of elements
                    arg_size = res_arg[idx-1] # array size in bytes
                    for ele_idx in range(array_size): 
                        ele_size = arg_size_dict['int'] # the size of the integer that indicate the string size
                        str_ele_size = self.dtype_convert(res_bin_rep[arg_byte_idx: arg_byte_idx + ele_size], 'bin', 'int') # the size of the string element
                        arg = self.dtype_convert(res_bin_rep[arg_byte_idx: arg_byte_idx + str_ele_size], 'bin', arg_fmt)
                        arg_byte_idx += ele_size


                        arg = self.dtype_convert(res_bin_rep[arg_byte_idx: arg_byte_idx + ele_size], 'bin', arg_fmt)
