        ...
        if get_arg:
            ...
            for idx, arg_fmt in enumerate(varg_fmt):
                if arg_fmt in arg_size_dict.keys():
                    ...
                    arg, _ = self.dtype_cvt(res_bin_rep[arg_byte_idx: arg_byte_idx + arg_size], 'bin', arg_fmt, arg_size)
                    arg_byte_idx += arg_size
                    res_arg.append(arg)
                    ...
                    num_cols = res_arg[idx-1] if not varg_fmt[idx-1].startswith('1d') and not varg_fmt[idx-1].startswith('2d') else res_arg[len(varg_fmt) -1- next(i for i, x in enumerate(reversed(varg_fmt)) if not x.startswith('1d') and not x.startswith('2d'))]

                    print(res_arg[len(varg_fmt) -1- next(i for i, x in enumerate(reversed(varg_fmt)) if not x.startswith('1d') and not x.startswith('2d'))])
