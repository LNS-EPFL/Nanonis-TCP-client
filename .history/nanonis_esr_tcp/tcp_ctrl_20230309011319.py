    ...
    def dtype_cvt(self, data, original_fmt, target_fmt, *arg):  
        ...
        dtype_cvt_dict = defaultdict(lambda: 
                                     ...
                                     # int32 to binary
                                      ...
                                      # from binary
                                      (original_fmt == 'bin' and target_fmt == 'str'): 
                                        np.frombuffer(data[4:], '>%dS' % np.frombuffer(data[0:4], '>i')[0]), # expression after '%' gives the size of the string in bytes
                                      (original_fmt == 'bin' and target_fmt == '1dstr'): 
                                        [].append(np.frombuffer(data, '>%dS' % len(data))),
                                      (original_fmt == 'bin' and target_fmt == 'int' or '1dint'): 
                                        np.frombuffer(data, '>i'), 
                                      (original_fmt == 'bin' and target_fmt == 'uint16'): 
                                        np.frombuffer(data, '>H'),
                                      (original_fmt == 'bin' and target_fmt == 'uint32' or '1duint32'): 
                                        np.frombuffer(data, '>L'),
                                      (original_fmt == 'bin' and target_fmt == 'float32' or '1dfloat32'): 
                                        np.frombuffer(data, '>f'),
                                      (original_fmt == 'bin' and target_fmt == '2dfloat32'): 
                                        np.frombuffer(data, '>f').reshape(arg),
                                      (original_fmt == 'bin' and target_fmt == 'float64'): 
                                        np.frombuffer(data, '>d')
                                     })
