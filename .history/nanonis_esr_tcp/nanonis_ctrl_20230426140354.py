...
######################################## Follow Me Module #############################################
    ...
    def FolMePSPropsSet(self, auto_resume, use_own_basename, basename_size, basename, ext_VI_path_size, ext_VI_path, pre_meas_delay):
        ...
        self.tcp.print_err(res_err)
        gen_swp_props_df = pd.DataFrame({'Auto resume': init_settling_t,
                                         'Use own basename': max_slew_rate,
                                         'Basename size': num_steps, 
                                         'Basename': periods, 
                                         'External VI path size': self.tcp.tristate_cvt_2(autosave), 
                                         'External VI path': self.tcp.tristate_cvt_2(save_dialog),
                                         'Pre-measure delay (s)': settling_t},
                                        index=[0]).T
