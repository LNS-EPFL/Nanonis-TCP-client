...
######################################## Atom Tracking Module #############################################
    def AtomTrackCtrlSet(self, at_ctrl, status): #Modulation: 0; Controller: 1; Drift measurement:2
        ...
        self.tcp.print_err(res_err)
        at_ctrl_df = pd.DataFrame({'Atom tracking control': at_ctrl,
                                   'Modulation=0 Controller=1 Drift measurement=2': None,
    ...
    def AtomTrackStatusGet(sel, at_ctrl):
        body  = self.tcp.dtype_cvt(at_ctrl, 'uint16', 'bin')
        header = self.tcp.header_construct('AtomTrack.CtrlGet', 0)
        cmd = header + body

        self.tcp.cmd_send(cmd)
        _, res_arg, res_err = self.tcp.res_recv()

        self.tcp.print_err(res_err)
        at_ctrl_df = pd.DataFrame({'Atom tracking control \nModulation: 0\nController: 1\n Drift measurement: 2': at_ctrl,
                                   'Status': self.tcp.bistate_cvt(status)},
                                 index=[0]).T
        print('\n'+
              at_ctrl_df.to_string(header=False)+
