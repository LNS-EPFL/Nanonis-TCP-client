    ...
    def dtype_cvt(self, data, original_fmt, target_fmt): # data_size is the length of an 1d array (8 for example)or the number of rows and columns of a 2d array ([2,2] for example) 
        ...
        dtype_cvt_dict = defaultdict(lambda: None, 
                                     ...
                                     # to binary
                                      ...
                                      ('int'    , 'bin'): lambda: np.array(data, dtype = '>i'),
                                      ('uint16' , 'bin'): lambda: np.array(data, dtype = '>H'),
                                      ('uint32' , 'bin'): lambda: np.array(data, dtype = '>L'),
                                      ('float32', 'bin'): lambda: np.array(data, dtype = '>f'),
                                      ('float64', 'bin'): lambda: np.array(data, dtype = '>d'),
                                      # from binary
