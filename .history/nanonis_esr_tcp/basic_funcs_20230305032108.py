...
#################### basic functions for creating commands ####################
    ...
    def header_construct(self,command_name, body_size, res = True):
        ...
        self.header_bin_rep += dtype_convert(body_size, 'int', 'bin')         # boty size
        self.header_bin_rep += dtype_convert(1 if res else 0, 'uint16', 'bin') # send response back (1) or not (0)
        self.header_bin_rep += b'\x00\x00'
        return self.header_bin_rep

    # todo: send command
    def cmd_send(self, sk, data):
