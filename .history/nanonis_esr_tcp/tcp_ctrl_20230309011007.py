...
#################### basic functions for creating commands ####################
    ...
    def dtype_cvt(self, data, original_fmt, target_fmt, *arg):  
        ...
        dtype_cvt_dict = defaultdict(lambda: 
                                     {
                                     # to binary
                                      (original_fmt == 'str'     or '1dstr'                    and target_fmt == 'bin'): 
                                        np.array(data, '>S').tobytes(),
                                      (original_fmt == 'int'     or '1dint'                    and target_fmt == 'bin'): 
                                        np.array(data, '>i').tobytes(),
                                      (original_fmt == 'uint16'                                and target_fmt == 'bin'): 
                                        np.array(data, '>H').tobytes(),
                                      (original_fmt == 'uint32'  or '1duint32'                 and target_fmt == 'bin'): 
                                        np.array(data, '>L').tobytes(),
                                      (original_fmt == 'float32' or '1dfloat32' or '2dfloat32' and target_fmt == 'bin'): 
                                        np.array(data, '>f').tobytes(),
                                      (original_fmt == 'float64'                               and target_fmt == 'bin'): 
                                        np.array(data, '>d').tobytes(),
                                      # from binary
