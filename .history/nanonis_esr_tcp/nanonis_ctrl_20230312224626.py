...
######################################## Bias Spectroscopy Module #############################################
    ...
    def BiasSpectrStatusGet(self):
        ...
        self.tcp.print_err(res_err)
        if res_arg[0] == 1:
            print('A bias spectroscopy measurement is running......')
        else:
    ...
    def BiasSpectrPropsSet(self, save_all, num_sweeps, bw_sweep, num_pts, z_offset, auto_save, show_save_dialog = 2):
        ...
        props_df = pd.DataFrame(
                                {'Save all': self.tcp.tristate_cvt(save_all), 
                                 'Number of sweeps': self.tcp.tristate_cvt(num_sweeps), 
                                 'Backward sweep': self.tcp.tristate_cvt(bw_sweep), 
                                 'Number of points': self.tcp.tristate_cvt(num_pts), 
                                 'Z offset (m)': self.tcp.tristate_cvt(z_offset),
                                 'Autosave': self.tcp.tristate_cvt(auto_save), 
                                 'Show save dialog': self.tcp.tristate_cvt(show_save_dialog)},
                                 index=[0]).T.to_string(index=False)
        print(props_df.transpose())
    ...
    def BiasSpectrPropsGet(self):
        ...
        props_df = pd.DataFrame(
                                {'Save all': self.tcp.tristate_cvt(res_arg[0]), 
                                 'Number of sweeps': self.tcp.tristate_cvt(res_arg[1]), 
                                 'Backward sweep': self.tcp.tristate_cvt(res_arg[2]), 
                                 'Number of points': self.tcp.tristate_cvt(res_arg[3]), 
                                 'Number of channels': self.tcp.tristate_cvt(res_arg[5]),
                                 'Channels': self.tcp.tristate_cvt(res_arg[6]), 
                                 'Number of Parameters': self.tcp.tristate_cvt(show_save_dialog)},
