    ...
    def res_recv(self, *varg_fmt, get_header = True, get_arg = True, get_err = True):
        ...
        if get_arg:
            ...
            for idx, arg_fmt in enumerate(varg_fmt):
                if arg_fmt in ['int', 'uint16', 'uint32', 'float32', 'float64']:
                    arg, arg_size = self.dtype_cvt(res_bin_rep[arg_byte_idx: arg_byte_idx + arg_size], 'bin', arg_fmt)
                    arg_byte_idx += arg_size

                    res_arg.append(arg[0])

                elif arg_fmt == 'str': 
                    arg_size, len_int = self.dtype_cvt(res_bin_rep[arg_byte_idx: arg_byte_idx + arg_size_dict['int']], 'bin', 'int')
                    arg_byte_idx += len_int

                    arg, _ = self.dtype_cvt(res_bin_rep[arg_byte_idx: arg_byte_idx+arg_size[0]], 'bin', arg_fmt)
                    arg_byte_idx += arg_size[0]
