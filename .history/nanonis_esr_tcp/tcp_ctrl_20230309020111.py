...
#################### basic functions for creating commands ####################
    ...
    '''arg is for 2d array conversions. 
       for 2d arrays: arg should be a tuple in the form of (num_rows, num_cols), eg. (41, 2)
       for 1d string: arg should be an integer like 8
    '''
    def dtype_cvt(self, data, original_fmt, target_fmt, *arg):  
        ...
        #* binary to 1d string
            ...
            return str_array
        #* binary to 2d string
        elif original_fmt == 'bin' and target_fmt == '2dstr': 
            ele_idx = 0
            str_array= []
            for idx in range(arg):
                ele_size = np.frombuffer(data[ele_idx: ele_idx+4], '>i')[0]
                ...
                str_array.append(ele)
            return np.array(str_array).reshape(arg)
