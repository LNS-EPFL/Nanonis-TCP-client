    ...
    def dtype_cvt(self, data, original_fmt, target_fmt): # data_size is the length of an 1d array (8 for example)or the number of rows and columns of a 2d array ([2,2] for example) 
        ...
        dtype_cvt_dict = defaultdict(lambda: None, 
                                     ...
                                     # to binary
                                      ...
                                      # from binary
                                      ('bin', 'str'    ): lambda: np.frombuffer('>%ds' % len(data), data),
                                      ('bin', 'int'    ): lambda: np.frombuffer('>i', data), 
                                      ('bin', 'uint16' ): lambda: np.frombuffer('>H', data),
                                      ('bin', 'uint32' ): lambda: np.frombuffer('>L', data),
                                      ('bin', 'float32'): lambda: np.frombuffer('>f', data),
                                      ('bin', 'float64'): lambda: np.frombuffer('>d', data)
                                     })
