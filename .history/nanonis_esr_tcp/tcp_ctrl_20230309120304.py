...
#################### basic functions for creating commands ####################
    ...
    def dtype_cvt(self, data, original_fmt, target_fmt, *arg):  
        ...
        #* binary to string (expression after '%' gives the size of the string in bytes)
            ...
            return np.frombuffer(data, '>%dS' % arg)[0].decode('utf-8'), len(data)
        ...
        #* binary to float64
            ...
            return np.frombuffer(data, '>d'), len(data)
        ...
        if get_arg:
            ...
            for idx, arg_fmt in enumerate(varg_fmt):
                if arg_fmt in arg_size_dict.keys():
                    ...
                    arg = self.dtype_cvt(res_bin_rep[arg_byte_idx: arg_byte_idx + arg_size], 'bin', arg_fmt)[0]
                    arg_byte_idx += arg_size

                    res_arg.append(arg)

                elif arg_fmt == 'str': 
                    arg_int, len_int = self.dtype_cvt(res_bin_rep[arg_byte_idx: arg_byte_idx + arg_size_dict['int']], 'bin', 'int')
                    arg_byte_idx += len_int

                    arg, arg_size = self.dtype_cvt(res_bin_rep[arg_byte_idx: arg_byte_idx+arg_int[0]], 'bin', arg_fmt)
                    arg_byte_idx += arg_size

                    res_arg.append(arg)
                elif arg_fmt == '1dstr':
                    arg_size = res_arg[idx-1]
                    arg, _ = self.dtype_cvt(res_bin_rep[arg_byte_idx: arg_byte_idx + arg_size], 'bin', arg_fmt, (arg_size, 1))
                    arg_byte_idx += arg_size
