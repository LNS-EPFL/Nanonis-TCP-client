...
#################### basic functions for creating commands ####################
    ...
    def dtype_cvt_v1(self, data, original_fmt, target_fmt): # data_size is the length of an 1d array (8 for example)or the number of rows and columns of a 2d array ([2,2] for example) 
    ...
    def dtype_cvt_np(self, data, original_fmt, target_fmt): # data_size is the length of an 1d array (8 for example)or the number of rows and columns of a 2d array ([2,2] for example) 
        supported_formats = ['bin', 'str', 'int', 'uint16', 'uint32', 'float32', 'float64']
        dtype_cvt_dict = defaultdict(lambda: None, 
                                     {
                                     # to binary
    ...
    def array_dtype_cvt(self, data, original_fmt, target_fmt, array_size): # array size can be a integer like 7 (for 1d arrays) or a list [7,8] (for 2d arrays)
        ...
        array_cvt_dict = defaultdict(lambda: None,
                                     {
                                      # to binary
                                      ('1dstr'    , 'bin'): lambda: np.array(data, dtype = 'S').tobytes(byteorder = 'big'), 
                                      ('1dint'    , 'bin'): lambda: np.array(data, dtype = np.int32).tobytes(byteorder = 'big'),
                                      ('1duint32' , 'bin'): lambda: 
                                      ('1dfloat32', 'bin'): lambda: 
                                      ('1dfloat64', 'bin'): lambda: 
                                      ('2dfloat32', 'bin'): lambda:
                                      ('2dstr'    , 'bin'): lambda:  
                                      # from binary
                                      ('bin', '1dstr'    ): lambda: 
                                      ('bin', '1dint32'  ): lambda:  
    ...
    def header_construct(self,command_name, body_size, res = True):
        ...
        self.header_bin_rep += self.dtype_cvt_v1(body_size, 'int', 'bin')         # boty size
        self.header_bin_rep += self.dtype_cvt_v1(1 if res else 0, 'uint16', 'bin') # send response back (1) or not (0)
    ...
    def res_recv(self, *varg_fmt, get_header = True, get_arg = True, get_err = True):
        ...
        if get_header:
            res_header['commmand name'] = self.dtype_cvt_v1(res_bin_rep[0:32], 'bin', 'str').replace('\x00', '') # drop all '\x00' in the string
            res_header['body size'] = self.dtype_cvt_v1(res_bin_rep[32:36], 'bin', 'int')            
        ...
        if get_arg:
            ...
            for idx, arg_fmt in enumerate(varg_fmt):
                if arg_fmt in arg_size_dict.keys(): # arg_fmts that are: 'int', 'uint16', 'uint32', 'float32', 'float64'
                    ...
                    arg = self.dtype_cvt_v1(res_bin_rep[arg_byte_idx: arg_byte_idx + arg_size], 'bin', arg_fmt)
                    arg_byte_idx += arg_size                                           
                    res_arg.append(arg)
                    
                elif arg_fmt == 'str':
                    int_size = arg_size_dict['int'] # the size of an integer 32 that give the size of the following string
                    str_size = self.dtype_cvt_v1(res_bin_rep[arg_byte_idx: arg_byte_idx + int_size], 'bin', 'int') # the size of the string 
                    arg_byte_idx += int_size

                    arg = self.dtype_cvt_v1(res_bin_rep[arg_byte_idx: arg_byte_idx + str_size], 'bin', arg_fmt) # convert the string from binary
                    ...
                    for ele_idx in range(num_of_ele): 
                        ...
                        ele_size = self.dtype_cvt_v1(res_bin_rep[arg_byte_idx: arg_byte_idx + int_size], 'bin', 'int') # the size of the string element
                        arg_byte_idx += int_size

                        ele = self.dtype_cvt_v1(res_bin_rep[arg_byte_idx: arg_byte_idx + ele_size], 'bin', arg_fmt)
                    ...
                    for ele_idx in range(num_of_ele):
                        ele = self.dtype_cvt_v1(res_bin_rep[arg_byte_idx: arg_byte_idx + ele_size], 'bin', arg_fmt[2:]) # '2:' --> leaving out '1d' in the fmt string
                        arg_byte_idx += ele_size
                        num_1d.append(ele)
        ...
        if get_err:
            res_err['error status'] = self.dtype_cvt_v1(res_bin_rep[0:4], 'bin', 'uint32') # error status
            res_err['error body size'] = self.dtype_cvt_v1(res_bin_rep[4:8], 'bin', 'int') # error description size
            res_err['error description'] = self.dtype_cvt_v1(res_bin_rep[8:], 'bin', 'str') # error description
